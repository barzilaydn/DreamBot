...
def cross_assign(agents, jobs, score_Func=None, injective=False):
    """
    ...
    :param score_Func: a function for generating the score, if None given use distance
    :param injective: one-to-one: only assign one job per agent even if there are more agents than jobs.
    :return: table (list of lists) of agents and jobs
            format: [ [job, agent, score], [...], ...]
    """
    ...
    my_table = []
    # Build table
    for i, job in enumerate(e):
        row = []
        for j, agent in enumerate(m):
            if score_Func is not None:
                row.append(score_Func(job, agent))
            else:
                row.append(game.distance(job, agent))
        my_table.append(row)
...
# Munkres is an algorithm to cross-assign agents to jobs minimizing cost (Assignment problem):
